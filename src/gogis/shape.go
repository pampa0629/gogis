...
type ShapeFile struct {
	...
	geoPyms   map[int][]*shpPolyline // 矢量金字塔
	geoCounts map[int]int
}
...
func (this *ShapeFile) Open(filename string) bool {
	...
	shx.Read(ByteSlice(this.records))
...
func (this *ShapeFile) BuildVecPyramid() {
	bbox := NewRect2D(this.shpHeader.xmin, this.shpHeader.ymin, this.shpHeader.xmax, this.shpHeader.ymax)
	minLevel, maxLevel := calcMinMaxLevels(bbox, this.recordNum)
	// maxLevel += 3
	...
	this.geoPyms = make(map[int][]*shpPolyline)
...
// 抽稀一个层级
func (this *ShapeFile) thinOneLevel(level int, dis float64, wg *sync.WaitGroup) {
	defer wg.Done()

	geometrys := make([]*shpPolyline, this.recordNum)
	forcount := (int)(this.recordNum/ONE_DRAW_COUNT) + 1
...
func (this *ShapeFile) thinBatch(num int, level int, geometrys []*shpPolyline, dis float64, wg *sync.WaitGroup) {
	defer wg.Done()

	for i := 0; i < ONE_DRAW_COUNT && num < this.recordNum; i++ {
...
// 对一个geometry进行抽稀
func thinOneGeo(geo *shpPolyline, dis float64) *shpPolyline {
...
func thinOnePart(points []shpPoint, dis float64) (newPnts []shpPoint) {
	...
	for i := 1; i < len(points); i++ {
		if distanceSquare(points[pos].X, points[pos].Y, points[i].X, points[i].Y) > dis2 {
